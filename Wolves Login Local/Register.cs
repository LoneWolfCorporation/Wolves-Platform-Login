...
namespace Wolves_Login_Local
{
    public partial class Register : UserControl
    {
        ...
        {
            if (Program.WL.Panel_Portal.Visible == false)
            {
                Program.WL.Panel_Register.Visible = false;
                Program.WL.Panel_Register.SendToBack();
                Program.WL.Panel_Portal.Visible = true;
                Program.WL.Panel_Portal.BringToFront();
            }
        }
        ...
        {
            Thread.Sleep(1000);
            string RootFolder = @"%APPDATA%\WolvesLogin\Users\";
            ...
            if (!Directory.Exists(UserLoginFolder))
            {
                ...
                this.Invoke(new Action(() => {
                    ...
                    LogLabel.Text = "USER WAS REGISTERED SUCCESSFULLY. WELCOME TO WOLVES PLATFORM " + UpperCaseUser;
                }));
            }
            else if (Directory.Exists(UserLoginFolder))
            {
                this.Invoke(new Action(() => {
                    LogLabel.Visible = true;
                    LogLabel.ForeColor = Color.Red;
                    LogLabel.Text = "THIS USER IS ALREADY A MEMBER OF WOLVES PLATFORM.";
                }));
            }                        
        }
